...
public class AutoOrderController {
    ...
    public Object downloadExcel(HttpSession httpSession, RedirectAttributes redirectAttributes) {

        String fileName = "주문 리스트_" + LocalDate.now() + ".xlsx";

        boolean auto = latestFileService.autoOrderListLoad();
        ...
        if (auto) {
            try {
                ...
                httpSession.setAttribute("errorMessage", "엑셀 파일 생성 실패: " + e.getMessage());
